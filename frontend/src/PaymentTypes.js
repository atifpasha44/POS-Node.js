...
const PaymentTypes = ({ setParentDirty, records, setRecords }) => {
  ...
  const validateForm = () => {
    ...
    } else if (!/^\d{4}$/.test(form.payment_code)) {
      errors.payment_code = 'Payment Code must be exactly 4 digits';
    }
    
    if (!form.payment_name.trim()) {
      errors.payment_name = 'Payment Name is required';
    } else if (!/^[A-Za-z\s]+$/.test(form.payment_name)) {
      errors.payment_name = 'Payment Name must contain only alphabetic characters';
    } else if (form.payment_name.length > 25) {
      errors.payment_name = 'Payment Name must not exceed 25 characters';
    }
    ...
    if (form.commission && !/^\d+(\.\d{1,2})?$/.test(form.commission)) {
      errors.commission = 'Commission must be a valid number';
    }
    
    if (form.setup_amount && !/^\d+(\.\d{1,2})?$/.test(form.setup_amount)) {
      errors.setup_amount = 'Setup Amount must be a valid number';
    }
    ...
    if (records && records.length > 0) {
      const isDuplicate = records.some((record, index) => 
        ...
        record.payment_code === form.payment_code
      );
      if (isDuplicate) {
        errors.payment_code = 'Payment Code already exists for this property';
      }
    }
  ...
  const handleExport = type => {
    ...
    if (type === 'Excel') {
      ...
      XLSX.writeFile(wb, 'PaymentTypes.xlsx');
    } else if (type === 'PDF') {
      const doc = new jsPDF();
      const columns = [
        'Property Code', 'Payment Code', 'Payment Name', 'Short Name', 'Reference', 
        'Display Seq', 'Commission', 'Setup Amount', 'Integration', 'Default', 'Status'
      ];
      const rows = exportData.map(rec => [
        ...
        rec.commission || '',
        rec.setup_amount || '',
        rec.integration_required ? 'Required' : 'Not Required',
        rec.default_payment ? 'Default' : 'Standard',
        rec.inactive ? 'Inactive' : 'Active'
      ]);
      autoTable(doc, { head: [columns], body: rows });
      doc.save('PaymentTypes.pdf');
    }
  };
    ...
    <div className="propertycode-panel" style={{
      ...
      <form ref={formRef} className="propertycode-form" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 32px',padding:'32px 32px 0 32px'}} autoComplete="off">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'24px'}}>
          ...
          {/* Payment Code */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Payment Code</label>
            <div style={{width:'80%'}}>
              <input
                type="text"
                ...
                onChange={e => {
                  ...
                  handleInputChange('payment_code', value);
                }}
                style={{
                  width:'100%',
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.payment_code ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px'
                }}
          ...
          {/* Payment Name */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Payment Name</label>
            <div style={{width:'80%'}}>
              <input
                type="text"
                ...
                onChange={e => {
                  ...
                  handleInputChange('payment_name', value);
                }}
                style={{
                  width:'100%',
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.payment_name ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px'
                }}
          ...
          {/* Reference Input */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Reference Input</label>
            <div style={{width:'80%'}}>
              <input
                type="text"
                value={form.reference_input}
                onChange={e => {
                  const value = e.target.value.slice(0, 20);
                  handleInputChange('reference_input', value);
                }}
                style={{
                  width:'100%',
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.reference_input ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px'
                }}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'24px'}}>
          ...
          {/* Commission */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Commission</label>
            <div style={{width:'80%'}}>
              <input
                type="text"
                ...
                style={{
                  ...
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.commission ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px'
                }}
                placeholder="Enter commission (e.g., 2.50)"
              />
          ...
          {/* Setup Amount */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Setup Amount</label>
            <div style={{width:'80%'}}>
              <input
                type="text"
                value={form.setup_amount}
                onChange={e => {
                  const value = e.target.value.replace(/[^0-9.]/g, '');
                  handleInputChange('setup_amount', value);
                }}
                style={{
                  width:'100%',
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.setup_amount ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px'
                }}
                placeholder="Enter setup amount (optional)"
              />
              {fieldErrors.setup_amount && (
                <div style={{ color: '#f44336', fontSize: '0.875rem', marginTop: '4px' }}>
                  {fieldErrors.setup_amount}
                </div>
              )}
          ...
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Integration Required</label>
            <div style={{width:'80%'}}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={form.integration_required}
                  onChange={e => handleInputChange('integration_required', e.target.checked)}
                  style={{ transform: 'scale(1.2)' }}
                />
                <span style={{ fontWeight: 'bold', color: '#222' }}>Requires Integration</span>
              </label>
              <small style={{ color: '#666', display: 'block', marginTop: '4px' }}>
                Check if payment type requires third-party integration
              </small>
            </div>
          </div>
          ...
          {/* Default Payment */}
          <div style={{display:'flex',alignItems:'center'}}>
            <label style={{width:'180px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Default Payment</label>
            <div style={{width:'80%'}}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={form.default_payment}
                  onChange={e => handleInputChange('default_payment', e.target.checked)}
                  style={{ transform: 'scale(1.2)' }}
                />
                <span style={{ fontWeight: 'bold', color: '#222' }}>Default Payment Type</span>
              </label>
              <small style={{ color: '#666', display: 'block', marginTop: '4px' }}>
                Check to set this as the default payment method
              </small>
            </div>
          </div>
          ...
          <div style={{display:'flex',alignItems:'center'}}>
            ...
            <div style={{width:'80%'}}>
              ...
              <small style={{ color: '#666', display: 'block', marginTop: '4px' }}>
                Check to mark this payment type as inactive
              </small>
            </div>
          </div>
        </div>
      ...
      {records && records.length > 0 && (
        <div style={{ padding: '20px 32px' }}>
          <h3 style={{ marginBottom: '16px', color: '#333' }}>Existing Payment Types</h3>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f5f5f5' }}>
                  ...
                  <th style={{ padding: '12px', textAlign: 'left', borderBottom: '2px solid #ddd' }}>Short Name</th>
                  <th style={{ padding: '12px', textAlign: 'left', borderBottom: '2px solid #ddd' }}>Commission</th>
                  <th style={{ padding: '12px', textAlign: 'center', borderBottom: '2px solid #ddd' }}>Integration</th>
                  <th style={{ padding: '12px', textAlign: 'center', borderBottom: '2px solid #ddd' }}>Default</th>
              ...
              <tbody>
                {records.map((record, index) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>{record.payment_code}</td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>{record.payment_name}</td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>{record.short_name}</td>
                    <td style={{ padding: '12px', borderBottom: '1px solid #eee' }}>{record.commission || '-'}</td>
                    <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #eee' }}>
                      {record.integration_required ? '✅' : '❌'}
                    </td>
                    <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #eee' }}>
                      {record.default_payment ? '✅' : '❌'}
                    </td>
                    <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #eee' }}>
                      <span style={{ 
                        padding: '4px 8px', 

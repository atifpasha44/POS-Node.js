...
const TaxCodes = ({ setParentDirty, records, setRecords }) => {
  ...
  const validateForm = () => {
    ...
    if (!form.tax_code.trim()) {
    ...
    } else if (form.tax_code.length > 10) {
      errors.tax_code = 'Tax Code must not exceed 10 characters';
    } else {
      // Check for duplicate tax codes (case insensitive)
      const isDuplicate = records && records.some((record, index) => {
        ...
        return record.tax_code.toLowerCase() === form.tax_code.toLowerCase();
      });
      if (isDuplicate) {
        errors.tax_code = 'Tax Code already exists';
      }
    }
    
    if (!form.tax_name.trim()) {
      errors.tax_name = 'Tax Name is required';
    } else if (form.tax_name.length > 100) {
      errors.tax_name = 'Tax Name must not exceed 100 characters';
    }
    
    if (!form.tax_percentage.toString().trim()) {
      errors.tax_percentage = 'Tax Percentage is required';
    } else {
      const percentage = parseFloat(form.tax_percentage);
      if (isNaN(percentage)) {
        errors.tax_percentage = 'Tax Percentage must be a valid number';
    ...
    if (!form.applicable_on.trim()) {
      errors.applicable_on = 'Applicable On is required';
    }
  ...
  const handleEdit = () => {
    ...
    setForm({
      tax_code: selectedRecord.tax_code || '',
      tax_name: selectedRecord.tax_name || '',
      tax_percentage: selectedRecord.tax_percentage || '',
      applicable_on: selectedRecord.applicable_on || '',
    ...
    <div className="propertycode-panel" style={{
      ...
      {showSavePopup && (
        <div style={{
          ...
          ✅ {lastAction === 'Add' ? 'Tax Code added successfully!' : 
               lastAction === 'Edit' ? 'Tax Code updated successfully!' : 
               lastAction === 'Delete' ? 'Tax Code deleted successfully!' : 
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px' }}>
                  <thead>
                    <tr style={{ background: '#f5f5f5' }}>
                      ...
                      <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>Tax Name</th>
                      <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'center' }}>Rate</th>
                      <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>Applicable On</th>
                  ...
                  <tbody>
                    {records.map((record, idx) => (
                      <tr key={idx} style={{ background: selectedRecordIdx === idx ? '#e3f2fd' : '#fff' }}>
                        <td style={{ border: '1px solid #ddd', padding: '8px' }}>{record.tax_code}</td>
                        <td style={{ border: '1px solid #ddd', padding: '8px' }}>{record.tax_name}</td>
                        <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'center' }}>{record.tax_percentage}%</td>
                        <td style={{ border: '1px solid #ddd', padding: '8px' }}>{record.applicable_on}</td>
      ...
      <div style={{
        ...
        <form ref={formRef} style={{
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'24px'}}>
            ...
            <div style={{display:'flex',alignItems:'center'}}>
              <label style={{width:'200px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Tax Code *</label>
              <input 
                type="text" 
                name="tax_code" 
                value={form.tax_code} 
                ...
                style={{
                  ...
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.tax_code ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px',
                  background:'#fff',
            ...
            {/* Tax Name */}
            <div style={{display:'flex',alignItems:'center'}}>
              <label style={{width:'200px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Tax Name *</label>
              <input 
                type="text" 
                name="tax_name" 
                value={form.tax_name} 
                ...
                style={{
                  ...
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.tax_name ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px',
                  background:'#fff'
                }} 
                maxLength="100"
                placeholder="e.g., Value Added Tax, Service Tax"
              />
              {fieldErrors.tax_name && <span style={{color:'red',fontSize:'0.98rem',marginLeft:'12px'}}>{fieldErrors.tax_name}</span>}
            </div>

            {/* Tax Percentage */}
            <div style={{display:'flex',alignItems:'center'}}>
              <label style={{width:'200px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Tax Percentage *</label>
              <div style={{width:'75%',display:'flex',alignItems:'center'}}>
                <input 
                  ...
                  style={{
                    ...
                    height:'36px',
                    fontSize:'1.08rem',
                    border: fieldErrors.tax_percentage ? '2px solid #f44336' : '2px solid #bbb',
                    borderRadius:'6px',
                    padding:'0 8px',
                    background:'#fff'
                  }} 
                ...
                />
                <span style={{marginLeft:'8px',fontSize:'1.15rem',fontWeight:'bold',color:'#666'}}>%</span>
              </div>
              {fieldErrors.tax_percentage && <span style={{color:'red',fontSize:'0.98rem',marginLeft:'12px'}}>{fieldErrors.tax_percentage}</span>}
            </div>

            {/* Applicable On */}
            <div style={{display:'flex',alignItems:'center'}}>
              <label style={{width:'200px',fontWeight:'bold',fontSize:'1.15rem',color:'#222'}}>Applicable On *</label>
              <select 
                name="applicable_on" 
                ...
                style={{
                  ...
                  height:'36px',
                  fontSize:'1.08rem',
                  border: fieldErrors.applicable_on ? '2px solid #f44336' : '2px solid #bbb',
                  borderRadius:'6px',
                  padding:'0 8px',
                  background:'#fff'
                }}
              >
                ...
                {applicableOnOptions.map((option, index) => (
                  <option key={index} value={option}>
                    {option}
                  </option>
                ))}
              </select>
              {fieldErrors.applicable_on && <span style={{color:'red',fontSize:'0.98rem',marginLeft:'12px'}}>{fieldErrors.applicable_on}</span>}
            </div>
          </div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',minHeight:0}}>
          <div style={{fontWeight:'bold',fontSize:'1.5rem',color:'#222',marginBottom:'16px'}}>
            Tax Codes ({records ? records.length : 0})
          </div>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.9rem',minWidth:'1000px'}}>
              <thead>
                <tr style={{background:'#f5f5f5',position:'sticky',top:0,zIndex:1}}>
                  ...
                  <th style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'left',fontWeight:'bold',background:'#f5f5f5',minWidth:'120px'}}>Tax Code</th>
                  <th style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'left',fontWeight:'bold',background:'#f5f5f5',minWidth:'200px'}}>Tax Name</th>
                  <th style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'center',fontWeight:'bold',background:'#f5f5f5',minWidth:'100px'}}>Rate (%)</th>
                  <th style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'left',fontWeight:'bold',background:'#f5f5f5',minWidth:'150px'}}>Applicable On</th>
                  ...
                  <th style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'center',fontWeight:'bold',background:'#f5f5f5',minWidth:'120px'}}>Effective To</th>
                </tr>
              </thead>
              <tbody>
                {records && records.length === 0 ? (
                  <tr>
                    <td colSpan="8" style={{border:'1px solid #ddd',padding:'20px',textAlign:'center',color:'#888',fontStyle:'italic',background:'#fff'}}>No tax codes found</td>
                  </tr>
                ) : (
                  records && records.map((record, index) => (
                    ...
                    >
                      ...
                      <td style={{border:'1px solid #ddd',padding:'12px 8px',fontWeight:'bold',color:'#1976d2'}}>{record.tax_code}</td>
                      <td style={{border:'1px solid #ddd',padding:'12px 8px'}}>{record.tax_name}</td>
                      <td style={{border:'1px solid #ddd',padding:'12px 8px',textAlign:'center',fontWeight:'bold',color:'#2e7d32'}}>{record.tax_percentage}%</td>
                      <td style={{border:'1px solid #ddd',padding:'12px 8px'}}>{record.applicable_on}</td>

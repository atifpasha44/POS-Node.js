...
const initialState = {
  ...
  item_name: '',
  item_sold: '',
  reset_at_daily_close: ''
};

export default function ItemSold({ setParentDirty, records, setRecords }) {
  ...
  useEffect(() => {
    if (form.outlet_code) {
      ...
      setItemCodes([
        { code: 'ITEM001', name: 'Grilled Chicken' },
        { code: 'ITEM002', name: 'Caesar Salad' },
        { code: 'ITEM003', name: 'Margherita Pizza' },
        { code: 'ITEM004', name: 'Chocolate Cake' }
      ]);
    } else {
      setItemCodes([]);
  ...
  useEffect(() => {
    if (form.item_code) {
      ...
      if (selectedItem) {
        setForm(f => ({ ...f, item_name: selectedItem.name }));
      }
    } else {
  ...
  const handleSave = async () => {
    ...
    const requiredFields = [
      ...
      { key: 'reset_at_daily_close', label: 'Reset Item at Daily Close' }
    ];
  ...
  const handleExport = (format) => {
    ...
    if (format === 'Excel') {
      const exportData = records.map(rec => ({
        ...
        'Item Sold': rec.item_sold,
        'Reset at Daily Close': rec.reset_at_daily_close
      }));
      const ws = XLSX.utils.json_to_sheet(exportData);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'ItemSold');
      XLSX.writeFile(wb, 'ItemSold.xlsx');
    } else if (format === 'PDF') {
      const doc = new jsPDF();
      doc.text('Item Sold Report', 20, 20);
      const columns = ['Property Code', 'Outlet Code', 'Item Code', 'Item Name', 'Item Sold', 'Reset at Daily Close'];
      const rows = records.map(rec => [
        rec.property_code, rec.outlet_code, rec.item_code, rec.item_name, rec.item_sold, rec.reset_at_daily_close
      ]);
      autoTable(doc, { head: [columns], body: rows, startY: 30 });
      doc.save('ItemSold.pdf');
    }
  };
    ...
    <div className="propertycode-panel" style={{
          ...
          <select
            ...
            onChange={e => {
              ...
              if (val === 'Add') {
                ...
                setAction('Search');
                setShowSelectModal(true);
                setSelectModalMessage('Search for Item Sold and click "View" to see full details in read-only mode.');
              }
            }}
      ...
      <form ref={formRef} className="propertycode-form" style={{
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '12px' }}>
                  <thead>
                    <tr style={{ background: '#e3e3e3' }}>
                      ...
                      <th style={{ padding: '6px 8px', fontWeight: 'bold', fontSize: '1rem' }}>Outlet</th>
                      <th style={{ padding: '6px 8px', fontWeight: 'bold', fontSize: '1rem' }}>Item</th>
                      <th style={{ padding: '6px 8px', fontWeight: 'bold', fontSize: '1rem' }}>Sold</th>
                  ...
                  <tbody>
                    {records.map((rec, idx) => (
                      <tr key={idx} style={{ background: idx % 2 ? '#f7f7f7' : '#fff' }}>
                        ...
                        <td style={{ padding: '6px 8px' }}>{rec.item_code}</td>
                        <td style={{ padding: '6px 8px' }}>{rec.item_sold}</td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <label style={{ width: '180px', fontWeight: 'bold', fontSize: '1.15rem', color: '#222' }}>
              Reset Item at Daily Close
            </label>
            <select
              name="reset_at_daily_close"
              value={form.reset_at_daily_close}
              onChange={handleChange}
              style={{
                width: '80%',
                height: '36px',
            ...
            {fieldErrors.reset_at_daily_close && (
              <span style={{ color: 'red', fontSize: '0.98rem', marginLeft: '12px' }}>
                {fieldErrors.reset_at_daily_close}
              </span>
            )}

...
const UnitOfMeasurement = ({ setParentDirty, records, setRecords }) => {
  ...
  const exportToExcel = () => {
    const exportData = records && records.length > 0 ? records.map(record => ({
      'UOM Code': record.uom_code,
      'UOM Name': record.uom_name,
      'Conversion Factor': record.conversion_factor || 'N/A',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'UnitOfMeasurement');
    XLSX.writeFile(wb, 'UnitOfMeasurement.xlsx');
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    const tableColumn = ['Code', 'Name', 'Conversion Factor', 'Status', 'Sequence'];
    ...
    doc.text('Unit of Measurement Report', 14, 15);
    doc.save('UnitOfMeasurement.pdf');
  };

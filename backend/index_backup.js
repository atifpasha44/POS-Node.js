...
const createItemCategoriesTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_ITEM_CATEGORIES (
    ...
    name VARCHAR(20) NOT NULL,
    alternate_name VARCHAR(20),
    item_department_code VARCHAR(4),
    item_department_name VARCHAR(20),
...
// Create IT_CONF_OUTSET table (Outlet Setup)
const createOutletSetupTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_OUTSET (
    id INT AUTO_INCREMENT PRIMARY KEY,
    applicable_from DATE NOT NULL,
    outlet_code VARCHAR(4) NOT NULL,
    outlet_name VARCHAR(30) NOT NULL,
    short_name VARCHAR(10),
    outlet_type ENUM('Restaurant','Bar','Both') DEFAULT 'Restaurant',
    bill_initial VARCHAR(2),
    outlet_setting JSON,
    options JSON,
    inactive BOOLEAN DEFAULT 0,
    created_by VARCHAR(64),
...
// Create IT_CONF_TAXCODE table
const createTaxCodesTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_TAXCODE (
    id INT AUTO_INCREMENT PRIMARY KEY,
    tax_code VARCHAR(3) NOT NULL UNIQUE,
    tax_name VARCHAR(25) NOT NULL,
    tax_name_alternate VARCHAR(25),
    tax_group_name VARCHAR(25),
...
// Create IT_CONF_TAXSTRUCTURE table
const createTaxStructureTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_TAXSTRUCTURE (
    id INT AUTO_INCREMENT PRIMARY KEY,
    tax_structure_code VARCHAR(6) NOT NULL UNIQUE,
    tax_structure_name VARCHAR(30) NOT NULL,
    outlet_code VARCHAR(4),
    menu_type VARCHAR(15),
    serial_number INT,
    short_tax VARCHAR(3),
    tax_code VARCHAR(3),
    calculation_type ENUM('Percentage','Amount') DEFAULT 'Percentage',
    amount DECIMAL(15,3) DEFAULT 0,
    target_tax INT DEFAULT 0,
    is_active BOOLEAN DEFAULT 1,
    created_by VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_by VARCHAR(64),
...
// Create IT_CONF_UOM table (Unit of Measurement)
const createUOMTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_UOM (
    id INT AUTO_INCREMENT PRIMARY KEY,
    uom_code VARCHAR(8) NOT NULL UNIQUE,
    uom_name VARCHAR(20) NOT NULL,
    container_unit VARCHAR(5),
    container_size DECIMAL(18,4),
    contained_unit VARCHAR(8),
    is_active BOOLEAN DEFAULT 1,
    created_by VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_by VARCHAR(64),
...
// Create IT_CONF_USER_GROUPS table
const createUserGroupsTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_USER_GROUPS (
    id INT AUTO_INCREMENT PRIMARY KEY,
    group_code VARCHAR(6) NOT NULL UNIQUE,
    group_name VARCHAR(20) NOT NULL,
    group_details VARCHAR(30),
    is_active BOOLEAN DEFAULT 1,
    created_by VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_by VARCHAR(64),
...
// Create IT_CONF_REASONS table (Reason Codes)
const createReasonCodesTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_REASONS (
    id INT AUTO_INCREMENT PRIMARY KEY,
    reason_code VARCHAR(4) NOT NULL UNIQUE,
    reason_name VARCHAR(20) NOT NULL,
    reason_type ENUM('Discount','Void','Return','Complimentary') NOT NULL,
    is_active BOOLEAN DEFAULT 1,
    created_by VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_by VARCHAR(64),
...
// Create IT_CONF_CCM table (Credit Card Manager)
const createCreditCardTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_CCM (
    id INT AUTO_INCREMENT PRIMARY KEY,
    card_code VARCHAR(4) NOT NULL UNIQUE,
...
// Create IT_CONF_ITEM_MASTER table
const createItemMasterTableSQL = `CREATE TABLE IF NOT EXISTS IT_CONF_ITEM_MASTER (
    id INT AUTO_INCREMENT PRIMARY KEY,
    select_outlets JSON,
    item_code VARCHAR(20) NOT NULL UNIQUE,
    item_name VARCHAR(50) NOT NULL,
    short_name VARCHAR(20),
    item_department VARCHAR(4),
    applicable_from DATE NOT NULL,
    inventory_code VARCHAR(20),
    alternate_name VARCHAR(50),
    tax_code VARCHAR(3),
    item_category VARCHAR(4),
    item_price_1 DECIMAL(10,2) DEFAULT 0,
    item_price_2 DECIMAL(10,2) DEFAULT 0,
    item_price_3 DECIMAL(10,2) DEFAULT 0,
    item_price_4 DECIMAL(10,2) DEFAULT 0,
    item_printer_1 VARCHAR(20),
    item_printer_2 VARCHAR(20),
    item_printer_3 VARCHAR(20),
    set_menu ENUM('Yes','No') DEFAULT 'No',
    item_modifier_group VARCHAR(20),
    unit VARCHAR(10),
    print_group VARCHAR(20),
    cost DECIMAL(10,2) DEFAULT 0,
    in_active BOOLEAN DEFAULT 0,
    item_logo VARCHAR(256),
    item_logo_url VARCHAR(256),
    created_by VARCHAR(64),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_by VARCHAR(64),
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
db.connect((err) => {
        ...
        db.query('ALTER TABLE IT_CONF_PROPERTY DROP INDEX property_code', (err) => {
          ...
          db.query('SHOW INDEX FROM IT_CONF_PROPERTY WHERE Key_name = "unique_property_date"', (err, results) => {
            if (err) {
              ...
              db.query('ALTER TABLE IT_CONF_PROPERTY ADD CONSTRAINT unique_property_date UNIQUE (property_code, applicable_from)', (err) => {
  ...
  // Create all other POS configuration tables
  db.query(createItemDepartmentsTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_ITEM_DEPARTMENTS table:', err);
    else console.log('✅ IT_CONF_ITEM_DEPARTMENTS table created successfully');
  });

  db.query(createItemCategoriesTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_ITEM_CATEGORIES table:', err);
    else console.log('✅ IT_CONF_ITEM_CATEGORIES table created successfully');
  });

  db.query(createOutletSetupTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_OUTSET table:', err);
    else console.log('✅ IT_CONF_OUTSET table created successfully');
  });

  db.query(createTaxCodesTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_TAXCODE table:', err);
    else console.log('✅ IT_CONF_TAXCODE table created successfully');
  });

  db.query(createTaxStructureTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_TAXSTRUCTURE table:', err);
    else console.log('✅ IT_CONF_TAXSTRUCTURE table created successfully');
  });

  db.query(createUOMTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_UOM table:', err);
    else console.log('✅ IT_CONF_UOM table created successfully');
  });

  db.query(createUserGroupsTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_USER_GROUPS table:', err);
    else console.log('✅ IT_CONF_USER_GROUPS table created successfully');
  });

  db.query(createReasonCodesTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_REASONS table:', err);
    else console.log('✅ IT_CONF_REASONS table created successfully');
  });

  db.query(createCreditCardTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_CCM table:', err);
    else console.log('✅ IT_CONF_CCM table created successfully');
  });

  db.query(createItemMasterTableSQL, (err) => {
    if (err) console.error('Error creating IT_CONF_ITEM_MASTER table:', err);
    else console.log('✅ IT_CONF_ITEM_MASTER table created successfully');
  });
              return;
            }
